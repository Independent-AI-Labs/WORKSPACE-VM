...
#
# Downloads and installs gcloud CLI into the checkout-owned boot dir.
# Containment: no unsanctioned use of $HOME or
...
if [[ "$(id -u)" == "0" ]]; then
    echo "Error: local gcloud installation must run as the checkout owner, not root" >&2
    exit 1
fi
