...
check_uv() {
    ...
    # Look for uv in .boot-linux only - no system alternative allowed
        ...
        log_info "Run bootstrap script first: ./agents/workspace/scripts/setup/bootstrap.sh"
        return 1
    fi
}
...
_register_aliases_in_shell() {
    ...
    local source_line='[ -f "'$ami_root'/workspace/scripts/shell/shell-setup" ] && . "'$ami_root'/workspace/scripts/shell/shell-setup"'
...
register_shell_aliases() {
    local setup_script="workspace/scripts/shell/shell-setup"
